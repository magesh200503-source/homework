...
// Создаем слушатель событий по удалению и выполнению тудушки

/* todosElement.addEventListener("click", (event) => {
    if (event.target.classList.contains("button-delete")) {
        deleteTodoById(todos, Number(event.target.closest(".todo").dataset.id));
        event.target.closest(".todo").remove();
    }
    if (event.target.classList.contains("button-complete")) {
        completeTodoById(
            todos,
            Number(event.target.closest(".todo").dataset.id),
        );
        event.target.closest(".todo").classList.toggle("completed");
    }
}); */
